...
int ia(int * mat, int nc, int nf, int rango) {
	...
	if (ind != -1) {
		//Sino, podemos hacer el cambio y eliminar todas las posibles combinaciones que estén juntas..
		permutar(mat, nf, nc, ind / nc, ind % nc, movMejor, rango, 0, 1);
...
int main(int argc, char* argv[]) {
	...
	filas = atoi(argv[4]);
	tam = columnas * filas;
	//La dimensión del grid será 1 x 1 siempre, un solo bloque.
	dim3 dimGrid(1,1);
	//Todos las columnas y filas en un mismo bloque.
	dim3 dimBlock(columnas, filas, 1);
			...
			if (ia(matriz, columnas, filas, rango) == 0) {
				...
				case 1: if (intento > columnas) {
					...
					intento = 1;
							...
							eliminaColumna(matriz, rango, filas, columnas, intento - 1);
							intento++;
						}
						break;
				case 2:
					...
					intento = 1;
							...
							eliminaFila(matriz, rango, filas, columnas, intento - 1);
							intento++;
						}
						break;
				case 3: if (dificultad < 3) {
					...
					bomba++;
							...
							giraCuadrados(matriz, filas, columnas);
							intento++;
						}
						break;
				}
				...
				if (strcmp(acc, "salir") == 0 || strncmp(acc, "9 1 ", 4) == 0 || strncmp(acc, "9 2 ", 4) == 0 ||
						...
						if (x > 0 && x <= columnas) {
							if (y > 0 && y <= filas) {
								...
								if (mov == 'u' || mov == 'U' || mov == 'r' || mov == 'R' ||
									...
									//Permuta las celdas deshaciendo el cambio si no tiene combinaciones posibles 
									//borrando todos los diamantes que estén juntos en el tablero.
									permutar(matriz, filas, columnas, y - 1, x - 1, mov, rango, 0, 1);
								}
								else {
					...
					if (strncmp(acc, "9 1 ", 4) == 0) {
						...
						if (num > columnas || num <= 0) {
							...
							eliminaColumna(matriz, rango, filas, columnas, num - 1);
						}
					...
					if (strncmp(acc, "9 2 ", 4) == 0) {
						...
						if (dificultad >= 2) {
							...
							if (num > filas || num == 0) {
								...
								eliminaFila(matriz, rango, filas, columnas, num - 1);
							}
						}
						else {
