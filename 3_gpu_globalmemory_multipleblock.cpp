...
//M�ximo n�mero de ancho de la tesela, puesto que el m�ximo est� en 32*32 = 1024
#define MAX 32
...
//Rango: 4, 6, 8
//Filas, columnas y la multiplicaci�n de �stas, el tama�o.
...
//Pone por defecto la fuente a la t�pica salida de consola.
void clearUnderlined() {
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 7);
}

//Elimina la columna n�mero colum (empezando desde 0 hasta nc - 1)
...
//Elimina la fila n�mero fila (empezando desde 0 hasta nf - 1)
void eliminaFila(int * mat, int rango, int nf, int nc, int fila)
{
	...
	//Generamos n�meros al azar en la primera fila.
	for (int i = 0; i < nc; i++) {
		mat[i] = (rand() % rango) + 1;
	}
}

//Devuelve un n�mero natural (mayor que 0) indicando la cantidad de diamantes que se pueden eliminar (en horizontal y vertical)
int comportamientoCelda(int * mat, int nf, int nc, int celda) {
	//Y es la fila, X es la columna.
	int y = celda / nf, x = celda % nc;
	int color = mat[celda];
	int i;
	//Almacenamos en un array el n�mero de 
	...
	int ret = 0, aux;
	//Buscamos cual de las dos dimensiones (arriba y abajo o derehca y izquierda) a ver cual casa m�s.
	for (int k = 0; k < 2; k++) {
		aux = seguidos[k] + seguidos[k + 2];
		//Si tenemos m�s de un coincidente (1-1, 2-0, 0-2 en adelante) obtenemos la mayor.
		if (aux >= 2) {
			if (aux > ret) {
				ret = aux;
			}
...
void cambiarColorEnFuncionDeDiamante(int n) {
	...
	*/
	...
	case 4: changeUnderlined(/*170*//*175*/160);
		break;
	case 5: changeUnderlined(/*68*/79/*64*/); //CAMBIADO DE MARR�N A GRANATE
		break;
...
//Calcula el rango en funci�n de su dificultad. 
...
//Comprueba toda la matriz y elimina todos los movimientos que est�n actualmente.
...
int comprobarYEliminar(int * mat, int nf, int nc, int rango, int saltarEliminacion) {
	...
	int mejor = 0;
	//Indicamos el �ndice de donde est� la celda que tiene la mejor opci�n.
	int indMejor = -1;
	for (int i = 0; i < nf*nc; i++) {
		//Obtenemos cada valor.
		aEliminar[i] = comportamientoCelda(mat, nf, nc, i);
		//Si la combinaci�n actual es mejor que la que tenemos actualmente, actualizamos el valor y el �ndice.
	...
	if (indMejor > -1) {
		//Si no se salta la eliminaci�n...
		if (saltarEliminacion == 0) {
			//Eliminamos cada diamante empezando por arriba, para que no perdamos �ndices de diamantes que si se deben eliminar.
			for (int i = 0; i < nf*nc; i++) {
				if (aEliminar[i] > 0) {
...
//Deber� devolver 1 si se realiza un cambio para eliminar diamantes, 0 sino.
//Intercambia entre dos valores, el de la posici�n fil, col, el movimiento mov.
//Si esta automatico (!= 0), solo se permuta, sin hacer comprobaciones
//Si est� borrarSolo, eliminar� los diamantes de todo el tablero.
int permutar(int * mat, int nf, int nc, int fi, int col, char mov, int rango, int automatico, int borrarSolo) {
	...
	case 'U':
		if (fi > 0) {
			...
			//Si est� en modo manual, si el movimiento no genera una combinaci�n, se deshace.
			if (automatico == 0) {
				if (comportamientoCelda(mat, nf, nc, (fi - 1)*nc + col) > 0 ||
					comportamientoCelda(mat, nf, nc, fi*nc + col) > 0) {
					//Si genera la combinaci�n y se desean eliminar todos los posibles, se eliminan.
					if (borrarSolo == 1) {
	...
	case 'r':
	case 'R':
		//De igual manera ocurre en los dem�s posibles movimientos.
		if (col < nc - 1) {
			aux = mat[fi*nc + col + 1];
...
//ATENCI�N: M�todo muy poco eficiente. Mejor utilizar la de la CPU.
//NO TIENE UTILIDAD EN EL FLUJO DE EJECUCI�N.
__global__ void permutar_MG(int * mat, int nf, int nc, int fi, int co, char mov) {
	...
	int colcam = col;
	int filcam = fil;
	//Obtenemos solo la cruz de los diamantes m�s cercanos a la celda a cambiar (los diamantes de arriba, derecha, abajo e izquierda).
	if ((col < co + 2 && col > co - 2) || (fil < fi + 2 && fil > fi - 2)) {
		switch (mov) {
		case 'u':
		case 'U':
			if (fi > 0) {
				//Si la celda que se ha seleccionado ha pulsado arriba, la posicion con la que cambia var�a (- 1 fila)
				if (co == col && fi == fil) {
					filcam--;
				}
				//En cambio, si est� ejecutando este flujo y no es la celda que se quiere eliminar, es una de sus vecinas,
				//El efecto es el contrario, significa que se tendr� que cambiar con la fila de abajo. (+ 1 fila)
				else {
					filcam++;
				}
			}
		...
		//Si las columnas cambiadas y las filas cambiadas coinciden con la original (solo dos lo har�n, que ser�n 
		//la celda que se quiere cambiar y la celda con la que se cambiar�), entonces se permutan.
...
//Obtiene el n�mero del stream del archivo hasta encontrarse con ';'
...
int cargarNumeroFile(FILE * archivo) {
	...
	while (continuar) {
		...
		if (caracter != ';') {
			...
			if (num == -1) {
				//Obtenemos el c�digo ASCII del car�cter
				num = (int)caracter;
...
//Retorna 1 si se ha podido cargar con �xito.
int cargar() {
	FILE *archivo;
	...
	if (archivo != NULL)
	{
		*tipoEjecucion = fgetc(archivo);
		...
		//Nos quitamos el ; que est� entre medias.
		fgetc(archivo);
...
int * getCoordenadas(char * cad, int inicio) {
	//Aqu� guardaremos las coordenadas en memoria din�mica.
	int * coordenadas;
	...
	for (int j = 0; j < 2; j++) {
		...
		while (!encontrado) {
			*car = cad[i];
			//Si el caracter es un n�mero...
			if (*car >= '0' && *car <= '9') {
				//Si no tenemos a�n ning�n valor, ponemos las unidades directamente.
				if (coordenadas[j] == -1) {
...
//Obtiene un n�mero que se encuentra a partir del car�cter n�mero "inicio" en la cadena "cad"
...
//Elimina los diamantes que sean coincidentes a una �nica celda. S�LO se elimina 1 combinaci�n,
...
void eliminarCoincidentes(int * mat, int nf, int nc, int celda, int rango) {
	...
	//Miramos hacia arriba. Solo tenemos que mirar la posici�n de la celda actual, porque los dem�s han bajado.
	//Seguimos eliminando hasta que ya no est� de ese color.
...
//en las filas o columnas �NICAMENTE que son afectadas por el usuario. Si es un movimiento horizontal, involucra a
//dos columnas y una fila y si es vertial una columna y dos filas. Mediante foc indicamos si hay que comprobar
//otra fila o columna (0,1) y con aux indicamos qu� fila o columna tenemos que comprobar.
__global__ void eliminarDiamante_MG(int * mat, int nc, int nf, int colsel, int filsel, int foc, int aux, int rango) {
	...
	//Valor en el vector.
	int dim = fil * nc + col;
	//Si el hilo est� en la columna o fila indicada (incluso en la auxiliar) se pasa a comprobar.
	if (col == colsel || fil == filsel || (foc == 0 && fil == aux) || (foc == 1 && col == aux)) {
		//Realiza las comprobaciones igual que lo hac�a en la CPU.
		int color = mat[dim];
		int i;
		int seguidos[4]; //0 arriba, 1 derecha, 2 abajo, 3 izquierda.
		seguidos[0] = 0;
		...
		while (continuar) {
			...
			//Si seguimos dentro del tablero y el color de arriba es igual al de la celda se le a�ade uno, sino, se deja de comprobar.
		...
		__syncthreads();
		if (ret == 1) {
			//Indicamos que lo hemos eliminado como 0, la CPU se encargar� de actualizar las posiciones.
			mat[dim] = 0;
		}
	}
}

//Elimina una columna "colum" (bomba 1) mediante GPU y las dem�s se desplazan a la derecha.
__global__ void eliminaColumna_MG(int * mat, int rango, int nf, int nc, int colum) {
	...
	int col = blockIdx.x * blockDim.x + threadIdx.x;
	int fil = blockIdx.y * blockDim.y + threadIdx.y;
	//Dimensi�n dentro del vector
	int dim = fil * nc + col;
	//Valor nuevo a poner en la celda actual.
	int nuevo;
	//Comprobamos solo las columnas que ser�n movidas.
	if (col <= colum) {
		...
		//Despu�s de que todos hayan hecho las comprobaciones, se ponen los nuevos valores en la matriz.
		__syncthreads();
		mat[dim] = nuevo;
	}
}
...
__global__ void eliminaFila_MG(int * mat, int rango, int nf, int nc, int fila)
{
	...
	int dim = fil * nc + col;
	int nuevo;
	//Si estamos en las filas que se mover�n/eliminar�n.
	if (fil <= fila) {
		if (fil != 0) {
...
//Genera n�meros aleatorios cuando la matriz tiene ceros creados por parte de la GPU.
void actualizarCeros(int * mat, int nc, int tam, int rango) {
	//Tomar� el valor de la celda actual, ya que si tiene un 0 pasar� la columna a bajar, por lo que tendr� nuevo valor.
	int aux;
	//Tiene el valor de cuantas filas se desplaza arriba, ya que si hay una columna con muchos 0, el nuevo valor
	//de la celda no puede ser 0, debe ser el de m�s arriba o, si se llega al final, un nuevo valor.
	int j;
	...
	for (int i = tam; i >= 0; i--) {
		...
		while (mat[aux] == 0) {
			//Si estamos en la �ltima fila, entonces generamos un n�mero al azar.
			if (aux < nc) {
				...
				if (aux - nc * j >= 0) {
					...
					mat[aux - nc * j] = 0;
					//Pasamos a mirar una fila m�s arriba por si vuelve a ser 0.
					j++;
				}
				//Sino, si estamos en la �ltima columna, generamos un n�mero aleatorio.
				else {
					mat[aux] = (rand() % rango) + 1;
					break;
				}
...
__global__ void giraCuadrados_MG(int * mat, int nf, int nc) {
	...
	if (col < gpv * 3 && fil < gph * 3) {
		int nuevo;
		//CG y FG son los �ndices de la columna girada (CG) y la fila girada (FG)
		//(0,0)(1,0)(2,0)
		//(0,1)(1,1)(2,1)
		//(0,2)(1,2)(2,2)
		//Se hace as� para que todos los subcuadrados 3x3 tengan el mismo c�digo.
		int cg, fg;
		//Se obtiene la posici�n del subcuadrado.
		cg = col % 3;
		fg = fil % 3;
...
int ia(int * mat, int nc, int nf, int rango) {
	//Contadores.
	int i, j;
	//columna, fila y n�mero de diamantes que tiene alrededor de igual color.
	int col, fil, iguales;
	char mov[4] = { 'U', 'R', 'D', 'L' };
	//Matriz con los mejores posibles movimientos con los 4 combinaciones.
	int * mejor;
	//Este ser� el mejor movimiento entre arriba, derecha, abajo, izquierda.
	char movMejor = 'n';
	mejor = (int*)malloc(nf*nc * sizeof(int));
	//Inicializamos a 0 cada mejor combinaci�n.
	for (i = 0; i < nc*nf; i++) {
		mejor[i] = 0;
	}
	//�ndice de la mejor posibilidad.
	int ind = -1;
	//Para cada celda...
	for (i = 0; i < nc * nf; i++) {
		...
		for (j = 0; j < 4; j++) {
			...
			//Nos quedamos con la combinaci�n
			iguales = comportamientoCelda(mat, nf, nc, i);
			//Deshacemos el cambio (backtracking)
			permutar(mat, nf, nc, fil, col, mov[j], rango, 1, 0);
			//Si es mejor combinaci�n que otro movimiento sobre la misma celda lo actualizamos.
			if (iguales > mejor[i]) {
				mejor[i] = iguales;
	...
	//Si el �ndice es -1, no hay posibles combinaciones con ning�n movimiento.
	if (ind != -1) {
		//Sino, podemos hacer el cambio y eliminar �nicamente esa combinaci�n.
...
//Obtiene la estructura dimBlock con la distribuci�n de los hilos para que corran en 4 bloques diferentes.
dim3 setupKernel(int filas, int columnas) {
	...
	//Se guarda el indice que cumple esta especificaci�n
...
int main(int argc, char* argv[]) {
	...
	//Semilla para obtener n�meros aleatorios
	srand(time(0));
	//Recogemos los argumentos pasados en su ejecuci�n.
	...
	filas = atoi(argv[4]);
	tam = columnas * filas;
	//La dimensi�n del grid ser� 2x2 siempre.
	dim3 dimGrid(2, 2);
	//Configuramos la distribuci�n de hilos en funci�n de las propiedades de la tarjeta.
	dim3 dimBlock = setupKernel(filas, columnas);
	//En funci�n de la dificultad,el rango de valores var�a.
	rango = calcularRango(dificultad);
	...
	//Si la ejecuci�n es autom�tica.
	if (strcmp(tipoEjecucion, "-a") == 0) {
		//Cogemos la bomba n�mero 1 al principio.
		int bomba = 1;
		//N�mero de intentos, para no tener todo el rato las bombas y que pueda acabarse el juego.
		int intento = 1;
		do {
			...
			if (ia(matriz, columnas, filas, rango) == 0) {
				switch (bomba) {
					//Si se han superado los intentos (ponemos como l�mite el n�mero de columnas, para que haya fin).
					//Cogemos otra bomba.
				case 1: if (intento > columnas) {
					...
					intento = 1;
						...
						break;
				case 2:
					//De igual manera comprobamos las diferentes bombas en funci�n del nivel.
					if (dificultad < 2) {
		...
		//Acci�n introducida por el usuario.
		char* acc;
			...
			do {
				...
				if (strcmp(acc, "salir") == 0 || strncmp(acc, "9 1 ", 4) == 0 || strncmp(acc, "9 2 ", 4) == 0 ||
					...
					if (strncmp(acc, "-", 1) == 0) {
						...
						x = coord[0]; //Columnas
						y = coord[1]; //Filas
									  //Miraremos adem�s en otra fila o columna, que es la que sufrir� el cambio. Ejemplo:
									  //Si estamos en la fila 3 y movemos arriba, adem�s de mirar esa columna, tenemos que
						...
						int aux;
						//Si las coordenadas est�n en el tablero sin salirse de �l
						if (x > 0 && x <= columnas) {
							if (y > 0 && y <= filas) {
								//Se obtiene el movimiento, presumiblemente el �ltimo car�cter
								char mov = acc[strlen(acc) - 1];
								//Si est� entre los movimientos v�lidos.
								if (mov == 'u' || mov == 'U' || mov == 'r' || mov == 'R' ||
									...
									//Obtenemos la fila y columna  en funci�n del movimiento (contando de 0 a nc-1 y 0 a nf-1).
									//Adem�s, se a�ade el valor de la fila o columna auxiliar.
									int colsel, filsel;
									if (mov == 'u' || mov == 'U') {
										colsel = x - 1;
										filsel = y - 2;
					...
					if (strncmp(acc, "9 1 ", 4) == 0) {
						//Obtenemos el n�mero de columna que eliminar�.
						num = getNum(acc, 4);
						//Comrpobamos que est� en el tablero.
					...
					if (strncmp(acc, "9 2 ", 4) == 0) {
						...
						if (dificultad >= 2) {
							//Cogemos el n�mero de fila a eliminar.
							num = getNum(acc, 4);
							//Comprobamos que est� entre las filas de la matriz.
							if (num > filas || num == 0) {
					...
					if (strcmp(acc, "cargar") == 0) {
						//Si se ha podido cargar con �xito
						if (cargar() == 1) {
